...
const CRITIQUE_BY_TYPE = {
  prd: [
    'Метрики успеха не операционализированы — непонятно, как и когда меряем.',
    'Non-goals не заданы: высок риск разрастания скоупа.',
    'Не описаны крайние случаи и состояния ошибок.',
  ],
  problem_statement: [
    'Проблема описана через решение, а не через боль пользователя.',
    'Нет данных о частоте и серьёзности проблемы — приоритет недоказуем.',
  ],
  jtbd: [
    'Силы (push/pull/anxiety/habit) не сбалансированы — habit/anxiety проработаны слабо.',
    'Outcomes не измеримы.',
  ],
  research_note: [
    'Выборка не описана — неясно, насколько выводы репрезентативны.',
    'Возможна предвзятость подтверждения в подборе источников.',
  ],
  strategy: [
    'Не сформулирован ключевой риск, способный обрушить ставку.',
    'Нет критерия, по которому стратегию признают провальной.',
  ],
  experiment: [
    'Не задан MDE и мощность — длительность теста под вопросом.',
    'Метрика-страж (guardrail) не определена.',
  ],
}
